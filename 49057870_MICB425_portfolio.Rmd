...
MG-RAST: gene calling and annotation by sequence similarity  
[MG_RAST](https://metagenomics.anl.gov/) 

Pfam: protein family database, uses Hidden markov model  
[Pfam](http://pfam.xfam.org/) 

MEGAN, CARMA, Phymm: For toxonomy-dependant binning  
...
TETRA: taxonomy-independant binning  
...
Phylogenetic gene anchors are used to determine the different OTUs present in a sample and to order them in a tree. 16s RNA is mostly used as phylogenetic anchor gene.
Functional gene anchors show the genes for important metabolic functions that are present in a certain environment. These can be used to determine the specific metabolic potential of a community in its respective environment.
...
Binning: build longer sequences from overlapping fragments and assign them to OTUs  
Approaches:  
-Taxonomy-dependent: Supervised  
	-sequenes get assigned to homologs in reference sequence databases, often using BLAST  
...
-Composition-dependant: Unsupervised  
...
- Assambling algorithm: Represent each read a vertex and detect overlapping vertices. Find correct assembly: NP- Hamiltonian path problem.-> gets too time expensive when amount of fragments is too high, as produced by second gen sequencing of metagenome. Solution: use k-mer words instead of the whole reads. -> Assembling can now be done in linear-time with Eulerian path.   
...
Opportunities: To infer the genomic/metabolic potential of whole communities. Fast and efficient process, algorithms. Algorithms are continously improving.  
...
DNA gets amplified by bridge amplificationo, fragments get sequenced by synthesis, in parallel.
